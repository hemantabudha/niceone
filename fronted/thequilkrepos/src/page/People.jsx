...
export default function People(){
 ...
 const [commentText, setCommentText] = useState({}); // Object to store comment text per post
  ...
  const textareaRef = useRef([]); // Create a ref to access the textarea
...
const handleCommentInputChange = (postId, e) => {
  ...
  question
        ...
        setquestion("")
        setmyquestion(prev => [response.data, ...prev]); 
  ...
  const handleadd=(e)=>{
    const token = localStorage.getItem("token"); 
      if (!token) {
       alert("you must login to create a discussion.")
        return;
      }
    setadd(true)
  }
...
<div className="usercontainer" onScroll={handleScroll} style={{width:"100%",height:"100%"}}>
     {add===false&&(<div className="createsearch" style={{display:"flex",justifyContent:"end",alignItems:"center",border:"none",marginTop:"5px",marginBottom:"5px"}}>
          <div className="creates" onClick={handleadd}>
            <FontAwesomeIcon icon={faPlus} className="newsadd" style={{border:"none", marginLeft: "7.5px"}}/>
            <span className="add">add discussion</span>
          </div>
        </div>)}  
        ...
        ref={textareaRef} 
      />
          <div style={{display:"flex",alignItems:"center",marginBottom:"10px"}}><button className="cancel" onClick={handlecancel}>Cancel</button><button className="upload" onClick={handlesubmit}>{loadingpost?"uploading...":"upload"}</button></div>
