...
export default function Follower(){
  ...
  <div className="sidedivterm" onClick={handletermandcondition}>
   <span className="para">Terms and Condition</span> 
      ...
      <div className="followingcontainer" onScroll={handleScroll}>
      ...
      { Array.isArray(followingdata)&& followingdata.length > 0 ? (followingdata &&followingdata.map((current, index) => {
              ...
              <div key={index} className="follodiv">
                ...
                <div className="upperfollow">
                 <button className="none1">{current.followercounts>2?<FontAwesomeIcon icon={faCrown} className="crowns"/>:""}</button><button className="one">{formatLikes(current.followercounts)}</button> <button className="none2">{current.followercounts>2?<FontAwesomeIcon icon={faStar} className="stars"/>:""}</button>
                </div>
